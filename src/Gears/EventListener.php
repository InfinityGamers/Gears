...
use Gears\Kit\Acrobat;
use Gears\Kit\Berserker;
use Gears\Kit\Kit;
...
class EventListener implements Listener{
        ...
        public function interact(PlayerInteractEvent $e){
                ...
                if($this->core->getVault()->isKitEnabled($player)){
                        $this->core->getKitManager()->callEvent([
                            'Player' => $e->getPlayer(),
                            'Item' => $e->getItem(),
                            'Block' => $e->getBlock()
        ...
        public function damage(EntityDamageEvent $e){
                ...
                if($target instanceof Player){
                        $kit = $this->core->getVault()->getPlayerKit($target);
                ...
                if($e instanceof EntityDamageByEntityEvent){
                        ...
                        if($cause instanceof Player and $target instanceof Player){
                                if($this->core->getVault()->isKitEnabled($cause)){
                                        $this->core->getKitManager()->callEvent([
                                            'Player' => $cause,
                                            'Target' => $target,
                                        ...
                                        $ability = $this->core->getVault()->getPlayerKit($cause);
        ...
        public function death(PlayerDeathEvent $e){
                $player = $e->getPlayer();
                if($this->core->getVault()->isKitEnabled($player)){
                        $this->core->getVault()->getPlayerKit($player)->onDeath($player);
                        $this->core->getVault()->setKitDisabled($player);
                        $e->setDrops([]);
                }
        }
        ...
        public function quit(PlayerQuitEvent $e){
                $player = $e->getPlayer();
                if($this->core->getVault()->isKitEnabled($player)){
                        $this->core->getVault()->getPlayerKit($player)->onQuit($player);
                }
        }
}